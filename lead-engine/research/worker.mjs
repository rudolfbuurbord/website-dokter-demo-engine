...
export async function runOne({db,capture,runId,apiKey,paid=false,fetcher=fetch}){
 ...
 try{
  ...
  if(!answer.id||!Number.isFinite(answer.usage?.prompt_tokens)||!Number.isFinite(answer.usage?.completion_tokens))throw new Error('MODEL_USAGE_UNKNOWN');
  ...
  completion={...token,error_code:error,output:{source_url:task.company.website,status:'REVIEW_REQUIRED',error_code:error,evidence:evidence||[],contacts:captured?.contacts||[],reason:'Research failed or incomplete; no website defect inferred.'}};
 }
