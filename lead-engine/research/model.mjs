...
export function validateReview(review,images){
 if(!['ELIGIBLE','INELIGIBLE','REVIEW_REQUIRED'].includes(review.status)||!Array.isArray(review.evidence)||!Array.isArray(review.strengths)||!Array.isArray(review.unknowns)||!review.holistic_impression||!review.reason)throw new Error('INVALID_REVIEW');
 if(review.evidence.some(e=>!e.finding||!Number.isInteger(e.screenshot_index)||!images[e.screenshot_index]))throw new Error('INVALID_EVIDENCE_REFERENCE');
